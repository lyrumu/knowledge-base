...
- 用户已选 Cloudflare Pages
- 仓库已有 [`.github/workflows/cloudflare-pages.yml`](file:///f:/Notes/.github/workflows/cloudflare-pages.yml)
- 待用户：
  1. 在 Cloudflare Dashboard 创建 Pages 项目并连 GitHub
  2. 配置 3 个 secret：`CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_PROJECT_NAME`
  3. 首次 push 触发自动部署
...
- 部署上 Cloudflare Pages（用户在调研 secret 设置）
- /works/ /life/ 页面实际内容（暂时占位）
- 中英切换（i18n）
- 高级感 UI（Phase B）