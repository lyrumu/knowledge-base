...
​![乘积化简](image/C++算法NOTE/屏幕截图%202025-11-07%20231654.png)
...
eg：
![滑动窗口题目哦](image/C++算法NOTE/屏幕截图%202025-11-01%20230151.png)
...
![合唱队型](image/C++算法NOTE/屏幕截图%202025-11-17%20162454.png)
...
二分可以用`lower_bound,upper_bound`等STL来写:
...
注意 此时若不是严格上升的 改成`upper_bound`就好了
...
![导弹拦截](image/C++算法NOTE/屏幕截图%202025-11-17%20210510.png)
...
![实时差分例题](image/C++算法NOTE/屏幕截图%202025-12-05%20151447.png)
...
![二维数组差分例题](image/C++算法NOTE/屏幕截图%202025-11-09%20215629.png)
...
![走方格](image/C++算法NOTE/屏幕截图%202025-12-05%20140621.png)
...
由不重复的数字`1-n`组成的所有排列中，按`字典序`从小到大求出`第m小`的排列:
...
`memset`主要用于**快速清空**或**初始化**很大的内存空间,比较高效.
...
利用**迭代器遍历**map中的键值对:
因为不一定知道起始的键 也不知道键之间的间隔 也不知道末尾的键
...
补充：对于频率统计的问题,`map`和`unordered_map`都可以直接`m[键]++`,都会自动初始化该键的值为0;
...
### <mark>priority_queue</mark>

> 一种能够随时`插入新数据`，并且随时知道`最大值或者最小值`，并且随时`删除`当下`最大值或最小值`的容器(优先队列)
...
> vector<>是一种数据结构，用`[]`就表示`vector<>`这种类型的数组;
> 用`()`就表示vector的一种构造元素个数的函数
